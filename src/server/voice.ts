...
export function transcribeAudio(audioFilePath: string): string | null {
  try {
    ...
    execSync(`ffmpeg -i ${shellEscape(audioFilePath)} -ar 16000 -ac 1 -y ${shellEscape(wavFile)} 2>/dev/null`, {
      ...EXEC_OPTS,
      timeout: 15000,
    });
    const result = execSync(
      `whisper-cli -m ${shellEscape(WHISPER_MODEL)} --language auto --no-timestamps ${shellEscape(wavFile)} 2>/dev/null`,
      {
        ...EXEC_OPTS,
        timeout: 60000,
      },
